export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(500).json({ ok: false, error: "Server init failed", details: String(e?.message || e) });
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    shipping_address_collection: {
      allowed_countries: ["IT", "FR", "DE", "AT", "ES", "NL", "BE", "LU", "CH"],
    },
